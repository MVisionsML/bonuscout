...
export default function HeroSection() {
  ...
  return (
    <>
      {/* Scrolling ticker */}
      <div className="bg-[#0a0c11] border-b border-[#151720] overflow-hidden">
        <div className="animate-ticker flex whitespace-nowrap py-2">
          {TICKER_ITEMS.map((item, i) => (
            <span key={i} className="text-[10px] uppercase tracking-[0.2em] text-gray-600 mx-6 flex-shrink-0">
              {item} <span className="text-[#F5A623]/40 mx-2">◆</span>
            </span>
          ))}
        </div>
      </div>
      ...
      {/* Hero */}
      <section className="relative bg-[#080a0f] overflow-hidden" style={{ minHeight: '85vh' }}>
        <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" />

        <div className="relative z-10 flex flex-col items-center justify-center text-center px-4" style={{ minHeight: '85vh' }}>
          {/* Badge */}
          <div className="mb-8">
          ...
          {/* Subtitle */}
          <p className="text-[11px] sm:text-xs uppercase tracking-[0.25em] text-gray-500 mb-10 max-w-lg">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-14">
            <Link href="#casinos"
              className="bg-[#F5A623] hover:bg-[#E09520] text-black font-black text-sm uppercase tracking-wider px-8 py-4 rounded-xl transition-colors">
              View Top Bonuses
            </Link>
            <Link href="/reviews"
              className="border border-[#252830] hover:border-[#F5A623]/40 text-gray-400 hover:text-white font-bold text-sm uppercase tracking-wider px-8 py-4 rounded-xl transition-all">
              Casino Reviews
            </Link>
          </div>
        ...
        {/* Bottom fade */}
        <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#0D0F14] to-transparent" />
      </section>
